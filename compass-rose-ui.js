 ...
 *               for wind and water.
 *  Fork 20/09/2019 (leonardotrp): new settings - arrow-type (ship/arrow) / arrow-color / locked
 */
  ...
  $.fn.compassRoseUI = function(options, arg) {
    ...
    return this.each(function() {
      ...
      this.settings = $.extend( {
        'arrow-direction' : 'out',
        'arrow-type' : null, // ship or default arrow
        'arrow-color': null, // default black
        'locked': false,
      ...
      function __drawShip(ctx, canvas) {
          // ship
          ctx.moveTo(0,0);
          ctx.lineTo(15,20);
      ...
      function __drawArrow(ctx, canvas) {
          // arrow
          ctx.moveTo(0, 0);
      ...
      this.drawArrow = function(coords) {
          ...
          var color = canvas.parentElement.settings['arrow-color'];
          if (color)
            ctx.fillStyle = color;
          ...
          if (canvas.parentElement.settings && canvas.parentElement.settings['arrow-type'] === 'ship')
            __drawShip(ctx, canvas);
          else
            __drawArrow(ctx, canvas);
      ...
      ctx.beginPath();
      ctx.arc(backcanv.width / 2, backcanv.height / 2, maxradius, 0, Math.PI * 2, true);
      ctx.closePath();
      ...
      for (var i = maxradius / 5; i <= maxradius; i += maxradius / 5) {
        ...
        ctx.arc(backcanv.width / 2, backcanv.height / 2, i, 0, 2 * Math.PI);
      }
      ctx.closePath();
      ctx.stroke();
      ...
      ctx.fillText('O', -(backcanv.width / 2 - fontpad), 5);
      ctx.fillText('L', (backcanv.width / 2 - fontsize), 5);

      var numAngles = 8;
      for (var i = 0; i < numAngles; i++) {
        var angleSize = 360 / numAngles;
        __setTextAngle(ctx, (angleSize * i));
        ctx.rotate(angleSize * Math.PI / 180);
      }

      function __setTextAngle(ctx, angle) {
        var txt = String(angle) + '°';
      ...
      frontcanv.locked = frontcanv.parentElement.settings['locked'];
